...
void b2ChainShape::CreateChainFromFloats(const float32 vertices[], int32 count){
	b2Vec2* verticesOut = new b2Vec2[count];

	for(int32 i = 0; i < count; i++)
		verticesOut[i] = b2Vec2(vertices[2*i], vertices[2*i + 1]);

	CreateLoop( verticesOut, count );
	delete verticesOut;
}
